...
describe("FileSystem", function() {
    ...
    describe("getFileList", function() {
        it("should return blank array for no files", function() {
            let fs = new FileSystem();

            expect(fs.getFileList()).to.deep.equal([]);
        });

        it("should properly return list of filenames", function() {
            let fs = new FileSystem();
            fs.writeFile("a", "");
            fs.writeFile("b", "");
            fs.writeFile("c", "");

            expect(fs.getFileList()).to.deep.equal(["a", "b", "c"]);
        })
    })
});
